...
public class PlatformAddressFactory {
     ...
     */
    private static PlatformAddress make(long value, long size) {
        if (value == 0) {
    ...
    public synchronized static PlatformAddress on(long value, long size) {
        if (value == 0) {
            return PlatformAddress.NULL;
        }
        long idx = value >> 5;
        for (int probe = 0; probe < MAX_PROBES; probe++) {
            PlatformAddress cachedObj = cache[(int)(idx + probe) & CACHE_MASK];
            if (cachedObj == null) {
                return cache[(int)(idx + probe) & CACHE_MASK] =
        ...
        return cache[(int)(idx + replacementIndex) & CACHE_MASK] =
            new PlatformAddress(value, size);
    }
    // END android-changed
